...
class Courses(Resource):
    def get(self):
        ...
        parser.add_argument('courseId', required=False)  # add args
        parser.add_argument('courseName', required=True)
        ...
        return {'data': data[args['courseName']]}, 200  # return data and 200 OK

    def post(self):
        parser = reqparse.RequestParser()  # initialize
        parser.add_argument('courseId', required=True)  # add args
        parser.add_argument('courseName', required=True)
        parser.add_argument('tags', required=False)
        args = parser.parse_args()  # parse arguments to dictionary
        ...
        if args['courseId'] in list(data['courseId']):
            return {
                'message': "'{args['courseId']}' already exists."
            }, 409
        else:
            ...
            data.to_csv('courses.csv', index=False)  # save back to CSV
            return {'data': data.to_dict()}, 200  # return data with 200 OK

    def put(self):
        parser = reqparse.RequestParser()  # initialize
        parser.add_argument('courseId', required=True)  # add args
        parser.add_argument('courseName', required=True)  # add args
        parser.add_argument('tags', required=False)
        args = parser.parse_args()  # parse arguments to dictionary
        ...
        if args['courseId'] in list(data['courseId']):
            ...
            # select our user
            user_data = data[data['courseId'] == args['courseId']]

            # update user's locations
            user_data['tags'] = user_data['tags'].values[0] \
                .append(args['tags'])
            ...
            return {
                'message': "'{args['courseId']}' user not found."
            }, 404
    ...
    def delete(self):
        ...
        if args['courseId'] in list(data['courseId']):
            ...
            data = data[data['courseId'] != args['courseId']]
            ...
            return {
                'message': "'{args['coursesId']}' user not found."
            }, 404
...
class Locations(Resource):
    ...
    def post(self):
        ...
        if args['locationId'] in list(data['locationId']):
            ...
            return {
                'message': "'{args['locationId']}' already exists."
            }, 409
        else:
    ...
    def patch(self):
        ...
        if args['locationId'] in list(data['locationId']):
            ...
            return {
                'message': "'{args['locationId']}' location does not exist."
            }, 404
...
api.add_resource(Courses, '/courses')  # add endpoints
